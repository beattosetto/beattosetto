...
def update_beatmap_action_script(action: ActionLog):
    ...
    info_logger = setup_logger('info_log', f'media/{action.log}', 'a', logging.INFO)
    debug_logger = setup_logger('debug_log', f'media/action_log/log_{action.id}_debug.log', 'a', logging.DEBUG)
    ...
    for beatmap in Beatmap.objects.all():
        ...
        if (request_data.status_code == 200) and (request_data.json() != []):
            try:
                ...
                log_two_handler(info_logger, debug_logger, logging.ERROR, f"Error while updating the metadata of {beatmap.title}[{beatmap.version}] :\n{str(e)}")
                failed += 1
        else:
